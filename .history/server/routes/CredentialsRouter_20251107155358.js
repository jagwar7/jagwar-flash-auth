...
router.get('/get', AuthProviderSwitcher, async(req, res)=>{
    try {
        ...
        const creds = await UserCredential.findOne({owner: req.user.id});
        if(!creds){
...
router.put('/update', AuthProviderSwitcher, async(req, res)=>{
    try {
        ...
        await creds.save();
        return res.status(200).send({success: true, msg: "Credentials saved successfully."});
    } catch (error) {
        console.error("Error updating credentials:", error);
        return res.status(500).json({msg: "There is an error while updating credentials, Contact Admin"});
    }
});
