...
router.get('/google/url', async(req, res)=>{
    try {
        ...
        if(!clientId){
            return res.status(400).json(msg: "INTERNAL SERVER ERROR: Missing ");
        }
...
router.get('/google/callback', async(req, res)=>{
    ...
    try {
        ...
        const ownerdb = userCredentialCollection.findOne({clientPublicKey: state});

        if(!ownerdb){
            return res.status(400).send("INTERNAL SERVER ERROR: Missing client ID");
        }
        ...
        */
        ...
        if(createOrUpdateInDb == false){
            return res.status(400).json({msg: createOrUpdateInDb.message});
        }
        //-----------------------------------------------------------------------------------
...
router.post('/local/signup', async(req, res)=>{
    ...
    try {
        ...
        if(createOrUpdateInDb === false){
            return res.status(400).json({message: createOrUpdateInDb.message});
        }
