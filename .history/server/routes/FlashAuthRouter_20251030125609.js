...
router.get('/google/callback', async(req, res)=>{
    try {
        ...
        let {googleClientId, googleClientSecret, clientMongoDbUri} = siteData;
        ...
        */
        ...
        const createOrUpdateInDb = await findOrCreate(Decrypt(clientMongoDbUri,userProfile);
        if(createOrUpdateInDb == false){
            renderHTML = formatErrorInHtml(createOrUpdateInDb.message, Decrypt(siteData.clientFrontEndURL), failurePage);
            return res.set('Content-Type', 'text/html').send(renderHTML);
        }
        //------------------------------------------------------------------------------------------------------
