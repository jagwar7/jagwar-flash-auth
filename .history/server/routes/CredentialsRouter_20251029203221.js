...
router.put('/update', AuthProviderSwitcher, async(req, res)=>{
    try {
        ...
        if(!creds){
            ...
            clientPublicKey= await bycrypt.hash(clientPublicKey, 10);
            clientMongoDbUri: await bycrypt.hash(clientMongoDbUri, 10);
            clientSecretKey: await bycrypt.hash(clientSecretKey, 10);
            googleClientId: await bycrypt.hash(googleClientId, 10);
            googleClientSecret: await bycrypt.hash(googleClientSecret, 10);
