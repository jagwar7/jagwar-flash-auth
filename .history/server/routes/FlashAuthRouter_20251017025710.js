...
router.get('/google/url', async(req, res)=>{
    try {
        ...
        if(!clientId){
            return res.status(400).json({success: false, message: "INTERNAL SERVER ERROR: Missing Client Public Key"});
        }
        ...
        if(!siteData){
            return res.status(400).json({success: false, message: "INTERNAL SERVER ERROR: Invalid ClientID or Missing credentials"});
        }
