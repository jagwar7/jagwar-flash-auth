...
router.post('/local/signup', async(req, res)=>{
    ...
    console.log(":::LOG:::  CLIENT ID RECVIED: ", clientId );
    const {name, email, password} = req.body;
    if(!name || !email || !password){
        return res.status(400).json({success: false, message: "INTERNAL SERVER ERROR: All the fields are required"});
    }
    console.log(":::LOG:::  NAME, EMAIL, PASSWORD : ", name, email , password );

    let ownerdb;
    try {
        const userCredentialCollection = req.db.model('UserCredentails', UserCredentials);
        ownerdb = userCredentialCollection.findOne({clientPublicKey: clientId});
        if(!ownerdb){
            console.log(":::LOG:::  ❌ MISSING OWNER DB: ");

            return res.status(400).json({message: "INTERNAL SERVER ERROR: Site could not recognized"});
        }
        console.log("ownerdb ✅ :", ownerdb);
        ...
        const createOrUpdateInDb = await findOrCreate(ownerdb.clientMongoDbUri, userProfile);                     // USER WILL BE CREATED OR CHECKED IF ALREADY EXISTS
        if(createOrUpdateInDb === false){
            return res.status(400).json({success: false, message: createOrUpdateInDb.message});
        }
        console.log("USER CREATION RESPONSE: ",createOrUpdateInDb);
        const userObj = {
