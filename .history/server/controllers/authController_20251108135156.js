...
const SignUpWithJWT = async (req, res) => {
    ...
    try {
        ...
        await user.save();
        const userObj = {name, email, authType: 'local', id: user._id};
...
const SignUpWithGoogle = async (req, res) => {
    ...
    try {
        ...
        if (user) {
            ...
            const jwtToken = generateJWT(userObj);
            return res.status(200).json({success: true, data:jwtToken, message: "User already exists. Please sign in." });
        }
        ...
        return res.status(500).json({success: false, data:jwtToken, message: "User already exists. Please sign in." });
    }
};
