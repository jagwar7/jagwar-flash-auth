...
const SignInWithJWT = async (req, res) => {
    console.log("JWT SIGN IN FUNCTION CALLED at", new Date().toISOString());
    const startTime = Date.now();
    ...
    try {
        ...
        console.log(`🔍 Found user ${email}, checking password at ${new Date().toISOString()}`);
        const isMatch = await bcrypt.compare(password, user.password);
        console.log(`🔍 Password comparison took ${Date.now() - startTime}ms, match: ${isMatch}`);
        if (!isMatch) {
            console.log(`🚫 Password mismatch for ${email} at ${new Date().toISOString()}`);
            return res.status(401).json({ msg: "Incorrect password, Please try again" });
        }
        console.log(`🔑 Password matched for ${email} in ${Date.now() - startTime}ms`);
        console.log(`🔑 Generating JWT for ${email} at ${new Date().toISOString()}`);
